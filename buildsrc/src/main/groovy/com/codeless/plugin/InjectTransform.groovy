...
import com.codeless.plugin.utils.ModifyClassUtil2
import groovy.io.FileType
import org.apache.commons.codec.digest.DigestUtils
import org.apache.commons.io.FileUtils
import org.apache.commons.io.IOUtils
import org.gradle.api.Project
import org.qiyi.video.svg.plugin.ClassPools
...
public class InjectTransform extends Transform {
    ...
    public void transform(
        ...
        ModifyClassUtil2.gClassPool = ClassPools.create(project,inputs)
...
//        Log.info("flavorAndBuildType ${flavorAndBuildType}")
        ...
        inputs.each { TransformInput input ->
            ...
            input.jarInputs.each { JarInput jarInput ->
                ...
                boolean  needModifyJar = isJarNeedModify(jarInput.file);
                com.codeless.plugin.utils.Log.info("========> needModifyJar：${jarInput.file.absolutePath} : ${needModifyJar}");
                if (needModifyJar) {
             ...
             */
            input.directoryInputs.each { DirectoryInput directoryInput ->
                File dest = outputProvider.getContentLocation(directoryInput.name, directoryInput.contentTypes, TransformManager.SCOPE_FULL_PROJECT, Format.DIRECTORY);
//                Log.info("dest dir  ${dest.absolutePath}")
                File dir = directoryInput.file
                if (dir) {
                    com.codeless.plugin.utils.Log.info("========> dir：${dir.absolutePath}");
        ...
        if (jarFile) {
            ...
            while (enumeration.hasMoreElements()) {
                ...
                if (entryName.endsWith(".class")) {
                    ...
                    boolean  shouldModify = shouldModifyClass(className)
                    //com.codeless.plugin.utils.Log.info("  ====>className : ${className}, ${shouldModify}");
                    if (shouldModify) {
            ...
            com.codeless.plugin.utils.Log.info("${jarFile.absolutePath} is modified");
    ...
    private static String filePath2Classname(String entryName) {
    ...
    public static File modifyClassFile(File dir, File classFile, File tempDir) {
        ...
        try {
            ...
            String className = filePath2Classname(classFile.absolutePath.replace(dir.absolutePath + File.separator, ""));
            boolean  shouldModify = shouldModifyClass(className)
            //println("--------->shouldModify: " + className +" , " + shouldModify)
            if (shouldModify) {
